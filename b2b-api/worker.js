...
__name2(getResetDate, "getResetDate");
function usageKey(ip) {
  return `ip:${ip}`;
}
__name(usageKey, "usageKey");
__name2(usageKey, "usageKey");
...
__name2(aggregateKey, "aggregateKey");
function monthKey() {
  const d = /* @__PURE__ */ new Date();
...
__name2(checkAndIncrementRateLimit, "checkAndIncrementRateLimit");
...
__name2(incrementAggregate, "incrementAggregate");
...
__name2(proxyStatic, "proxyStatic");
...
__name2(handleUsage, "handleUsage");
...
__name2(handleWidget, "handleWidget");
var SLUG_ALIASES = {
  "usa": "united-states",
  "us": "united-states",
  "u-s-a": "united-states",
...
__name2(normalizeCountrySlug, "normalizeCountrySlug");
async function handlePredict(url, rlHeaders) {
  ...
  function avgRange(layers2, key) {
    const ranges = layers2.filter((l) => l && l[key]).map((l) => l[key]);
...
__name2(handlePredict, "handlePredict");
async function handleCheckCompliance(url, rlHeaders) {
  ...
  const framework = tier === "strict" ? { name: "Strict (e.g. GDPR + national supplements)", basis: "Opt-in or documented legitimate interest with balancing test" } : tier === "moderate" ? { name: "Moderate (legitimate-interest B2B framework)", basis: "Legitimate interest grounds, opt-out mandatory" } : { name: "Permissive (anti-spam framework, e.g. CAN-SPAM)", basis: "Truthful headers + working opt-out" };
  const consent_required = tier === "strict" ? "Opt-in or documented legitimate interest assessment" : tier === "moderate" ? "Documented legitimate interest grounds, no prior consent required for B2B" : "No prior consent; sender ID + opt-out are sufficient";
  const requirements = [
  ...
  const cross_border_notes = sender ? `Sending from ${sender.country} (${sender.compliance_tier} tier) to ${recipient.country} (${recipient.compliance_tier} tier). Recipient jurisdiction governs \u2014 ${recipient.country}'s rules apply regardless of sender location.` : `Recipient jurisdiction governs cold email rules for ${recipient.country}. Sender location does not exempt recipient-side requirements.`;
  return jsonResponse({
...
__name2(handleCheckCompliance, "handleCheckCompliance");
function handleOpenAPI() {
  const spec = {
    ...
    paths: {
      ...
      "/v1/benchmarks": { get: { operationId: "getBenchmarks", summary: "Email performance benchmarks (74 countries \xD7 14 industries)", tags: ["Reference"], responses: { "200": { description: "Benchmark dataset" } } } },
...
__name2(handleOpenAPI, "handleOpenAPI");
...
__name2(handleAiPluginManifest, "handleAiPluginManifest");
...
__name2(handleMcpManifest, "handleMcpManifest");
var worker_default = {
